  ...
  with tf.Session() as sess:
    ...
    for wav_path in glob.glob(wav_dir + '/*.wav'):
      ...
      with open(wav_path, 'rb') as wav_file:
        allResult.append(wav_path.split('\\')[-1])
      ...
      for node_id in top_k:
        ...
        allResult.append(human_string)
        #print(human_string)
        score = predictions[node_id]
        #allResult.extend(score)
...
def label_wav(wav_dir, labels, graph, input_name, output_name, how_many_labels):
  ...
  return run_graph(wav_dir, labels_list, input_name, output_name, how_many_labels)
